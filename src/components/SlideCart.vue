...
<template>
  <aside
    ...
    class="cart-box fixed bg-slate-900 top-16 right-0 bottom-0 left-0 md:left-auto md:w-96"
  >
    <div class="p-4">
      ...
      <div
        ...
        :key="i"
        class="bg-gray-700 p-2 rounded-md flex justify-between items-center my-3"
      >
        <div>
          <img :src="p.image" alt="" class="h-10 w-auto min-w-10 rounded-sm" />
        </div>
        <div class="px-4">
      ...
      <div
        v-if="cartStore.cartList.length >= 1"
        class="space-y-3 text-center mt-6"
      >
        <p>
          Precio total:
